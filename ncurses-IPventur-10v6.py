...
def fenster(os_version,NmapOK,User):
    text = ("IPventur Python 1.0v6 by Norman Woeske")
    text2 = datetime.datetime.now()
    zeit = text2.strftime("%Y-%m-%d %H:%M:%S")
    h, w = screen.getmaxyx()
    if h <= 32 or w <= 110:
        print("\nSorry, but your windows terminal size is not h33 x w111. Program has been stopped.\n")    
        exit()
...
def main():
    ...
    fenster(os_version,NmapOK,User)
    outputformat=ausgabe(output,stechnic,head)
